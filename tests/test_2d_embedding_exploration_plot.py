...
from huddles import create_2d_embeddings_exploration_plot
...
def test_valid_input(random_embeddings_and_data):
    ...
    result = create_2d_embeddings_exploration_plot(embeddings, dataset)
...
def test_embeddings_invalid_column_count(random_embeddings_and_data):
    ...
    with pytest.raises(ValueError):
        create_2d_embeddings_exploration_plot(embeddings, dataset)
...
def test_invalid_hue(random_embeddings_and_data):
    embeddings, dataset = random_embeddings_and_data
    with pytest.raises(ValueError):
        create_2d_embeddings_exploration_plot(embeddings, dataset, hue="c")  # 'c' is not in the dataset
